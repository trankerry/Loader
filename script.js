...
function Calc() {
    ...
    if(convert.checked) {
		...
		document.getElementById('lbs').innerHTML = 'You are lifting ' + rounded + ' lbs.';
    }
	...
	for (let j = 0; j < dups.length; j++) {
        ...
        if (dups[j].value == 'images/25kg.svg' || dups[j].value == 'images/20kg.svg' || dups[j].value == 'images/15kg.svg' || dups[j].value == 'images/10kg.svg') {
            img.width = 120;
        }else {
            img.width = 70;
        }
